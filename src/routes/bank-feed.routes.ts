...
const uuidShapeRe = /^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$/i;

const categorizeSchema = z.object({
  body: z.object({
    // Either a UUID-shaped category ID (assigning) or null (un-categorising).
    categoryId: z
      .union([z.string().regex(uuidShapeRe, 'must be a UUID'), z.null()])
      .optional(),
